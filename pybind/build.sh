...
cd ${BIND_DIR}
clang++ -O3 -Wall -shared -std=c++17 -fPIC -I../ -I../base -I../src -I../programs/local/ $(python3 -m pybind11 --includes) chdb.cpp \
    -L${LIBCHDB_DIR} -lchdb -o chdb$(python3-config --extension-suffix)

LD_LIBRARY_PATH=${LIBCHDB_DIR} python3 -c "import chdb; res = chdb.queryToBytes('select 1112222222,555', 'CSV'); print(str(res.tobytes()))"
