...
public:
    ...
    virtual R visitNode(const ConstASTPtr &, C &)
    {
        throw Exception("Visitor does not supported this AST node.", ErrorCodes::NOT_IMPLEMENTED);
    }
#define VISITOR_DEF(TYPE) \
...
#undef VISITOR_DEF
    ...
    template <typename R, typename C>
    static R accept(ConstASTPtr && node, ASTVisitor<R, C> & visitor, C & context)
    {
        return accept(std::move(node), visitor, context);
    }
...
#undef VISITOR_DEF
    ...
    template <typename R, typename C>
    static R accept(ConstASTPtr & node, ConstASTVisitor<R, C> & visitor, C & context)
    {
#define VISITOR_DEF(TYPE) \
    if (node->getType() == ASTType::TYPE) \
    { \
