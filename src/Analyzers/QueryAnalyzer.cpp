...
{
public:
    // Void process(const ConstASTPtr & node) { return ASTVisitorUtil::accept(node, *this, {}); }
...
ScopePtr QueryAnalyzerVisitor::analyzeTable(ASTTableIdentifier & db_and_table, const QualifiedName & column_prefix)
{
...

    {
        ...
        if (storage_id.getDatabaseName() != "system" && !dynamic_cast<const StorageMemory *>(storage.get()))
    ...
    // get alias columns
    {
        ...
        if (!alias_columns->children.empty())
        {
            ...
            QueryNormalizer::Data normalizer_data(aliases, source_columns_set, false, context->getSettingsRef(), true);
    ...
    ASTTableJoin & table_join, ScopePtr left_scope, ScopePtr right_scope, const String & right_table_qualifier)
{
    ...
    if (table_join.on_expression)
    {
        ...
        auto array_join_exprs = extractExpressions(
            context,
            analysis,
