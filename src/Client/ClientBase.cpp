...
void ClientBase::runInteractive()
{
    ...
    if (load_suggestions)
    {
        ...
        if (client_context->getApplicationType() == Context::ApplicationType::CLIENT)
            suggest->load<Connection>(client_context, connection_parameters, config().getInt("suggestion_limit"), wait_for_suggestions_to_load);
        else if (client_context->getApplicationType() == Context::ApplicationType::LOCAL)
            suggest->load<LocalConnection>(client_context, connection_parameters, config().getInt("suggestion_limit"), wait_for_suggestions_to_load);
    }
    ...
    do
    {
        ...
        if (suggest && suggest->getLastError() == ErrorCodes::USER_SESSION_LIMIT_EXCEEDED)
        {
            ...
            suggest->load(*connection, connection_parameters.timeouts, config().getInt("suggestion_limit"), client_context->getClientInfo());
        }
