...
for version in 3.8 3.9 3.10 3.11 3.12 3.13 3.14; do
    ...
    # Python 3.13+ needs newer pyarrow (16.1.0 has no pre-built wheels for 3.13+)
    elif [ "$version" = "3.13" ] || [ "$version" = "3.14" ]; then
        python -m pip install setuptools tox pandas pyarrow twine psutil deltalake wheel
    else
        python -m pip install setuptools tox pandas pyarrow==16.1.0 twine psutil deltalake wheel
    fi
    pyenv shell --unset
done
