...
class FunctionExecutorConfig:
    ...
    def _register_string_functions(self):
        ...
        self._pandas_implementations["startswith"] = lambda s, pat: s.str.startswith(
            pat
        )
        ...
        self._pandas_implementations["find"] = (
            lambda s, sub, start=0, end=None: s.str.find(sub, start, end)
        )
        self._pandas_implementations["rfind"] = (
            lambda s, sub, start=0, end=None: s.str.rfind(sub, start, end)
        )
        self._pandas_implementations["count_matches"] = lambda s, pat: s.str.count(pat)

        # String manipulation
        self._pandas_implementations["replace"] = (
            lambda s, pat, repl, regex=True: s.str.replace(pat, repl, regex=regex)
        )
        self._pandas_implementations["slice"] = (
            lambda s, start=None, stop=None, step=None: s.str.slice(start, stop, step)
        )
        self._pandas_implementations["slice_replace"] = (
            lambda s, start=None, stop=None, repl=None: s.str.slice_replace(
                start, stop, repl
            )
        )
        self._pandas_implementations["pad"] = (
            lambda s, width, side="left", fillchar=" ": s.str.pad(width, side, fillchar)
        )
        self._pandas_implementations["center"] = (
            lambda s, width, fillchar=" ": s.str.center(width, fillchar)
        )
        self._pandas_implementations["ljust"] = (
            lambda s, width, fillchar=" ": s.str.ljust(width, fillchar)
        )
        self._pandas_implementations["rjust"] = (
            lambda s, width, fillchar=" ": s.str.rjust(width, fillchar)
        )
        ...
        self._pandas_implementations["split"] = (
            lambda s, pat=None, n=-1, expand=False: s.str.split(pat, n=n, expand=expand)
        )
        self._pandas_implementations["rsplit"] = (
            lambda s, pat=None, n=-1, expand=False: s.str.rsplit(
                pat, n=n, expand=expand
            )
        )
        self._pandas_implementations["partition"] = lambda s, sep=" ": s.str.partition(
            sep
        )
        self._pandas_implementations["rpartition"] = (
            lambda s, sep=" ": s.str.rpartition(sep)
        )
        self._pandas_implementations["cat"] = (
            lambda s, others=None, sep=None: s.str.cat(others, sep=sep)
        )
        self._pandas_implementations["join"] = lambda s, sep: s.str.join(sep)

        # String extraction
        self._pandas_implementations["extract"] = (
            lambda s, pat, expand=True: s.str.extract(pat, expand=expand)
        )
        self._pandas_implementations["extractall"] = lambda s, pat: s.str.extractall(
            pat
        )
        ...
        # Encoding
        self._pandas_implementations["encode"] = (
            lambda s, encoding="utf-8": s.str.encode(encoding)
        )
        self._pandas_implementations["decode"] = (
            lambda s, encoding="utf-8": s.str.decode(encoding)
        )

        # Normalization
        self._pandas_implementations["normalize"] = (
            lambda s, form="NFC": s.str.normalize(form)
        )
        self._pandas_implementations["translate"] = lambda s, table: s.str.translate(
            table
        )
    ...
    def _register_math_functions(self):
        ...
        self._pandas_implementations["clip"] = lambda s, lower=None, upper=None: s.clip(
            lower, upper
        )
    ...
    def _register_aggregate_functions(self):
        ...
        self._pandas_implementations["mode"] = lambda s: (
            s.mode().iloc[0] if len(s.mode()) > 0 else None
        )
        self._pandas_implementations["first"] = lambda s: (
            s.iloc[0] if len(s) > 0 else None
        )
        self._pandas_implementations["last"] = lambda s: (
            s.iloc[-1] if len(s) > 0 else None
        )
    ...
    def _register_datetime_functions(self):
        ...
        self._pandas_implementations["is_quarter_start"] = (
            lambda s: s.dt.is_quarter_start
        )
        ...
        # Date conversion
        self._pandas_implementations["to_period"] = lambda s, freq=None: s.dt.to_period(
            freq
        )
    ...
    def _register_pandas_only_functions(self):
        ...
        # Shift and diff
        self._pandas_implementations["shift"] = (
            lambda s, periods=1, fill_value=None: s.shift(
                periods, fill_value=fill_value
            )
        )
        self._pandas_implementations["diff"] = lambda s, periods=1: s.diff(periods)
        self._pandas_implementations["pct_change"] = lambda s, periods=1: s.pct_change(
            periods
        )

        # Ranking
        self._pandas_implementations["rank"] = (
            lambda s, method="average", ascending=True: s.rank(
        ...
        self._pandas_implementations["value_counts"] = (
            lambda s, normalize=False: s.value_counts(normalize=normalize)
        )
        self._pandas_implementations["duplicated"] = (
            lambda s, keep="first": s.duplicated(keep=keep)
        )
        self._pandas_implementations["drop_duplicates"] = (
            lambda s, keep="first": s.drop_duplicates(keep=keep)
        )
        ...
        self._pandas_implementations["interpolate"] = (
            lambda s, method="linear": s.interpolate(method=method)
        )
        ...
        self._pandas_implementations["between"] = (
            lambda s, left, right, inclusive="both": s.between(
        ...
        # Clipping
        self._pandas_implementations["clip_lower"] = lambda s, threshold: s.clip(
            lower=threshold
        )
        self._pandas_implementations["clip_upper"] = lambda s, threshold: s.clip(
            upper=threshold
        )

        # Binning
        self._pandas_implementations["cut"] = lambda s, bins, labels=None: pd.cut(
            s, bins, labels=labels
        )
        self._pandas_implementations["qcut"] = lambda s, q, labels=None: pd.qcut(
            s, q, labels=labels
        )
    ...
    def get_accessor_fallback_reason(
        self, accessor_method: str, **kwargs
    ) -> Optional[str]:
        """
    ...
    def register_pandas_implementation(
        self, function_name: str, implementation: Callable
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"FunctionExecutorConfig("
            f"default={self._default_engine.value}, "
            f"custom_mappings={custom_count})"
        )
